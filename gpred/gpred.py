...
def find_stop(stop_regex: Pattern, sequence: str, start: int) -> Union[int, None]:
    ...
    """
    # Search for stop codons starting from the start position
    # Must be in the same reading frame (multiple of 3 from start)
    for match in stop_regex.finditer(sequence, start):
        stop_pos = match.start(0)
        # Check if the stop codon is in the same reading frame
...
def has_shine_dalgarno(shine_regex: Pattern, sequence: str, start: int, max_shine_dalgarno_distance: int) -> bool:
    ...
    # If search start is negative, return False
    if shine_start < 0:
        return False
    
    # Search for Shine-Dalgarno motif in the specified region
    match = shine_regex.search(sequence, shine_start, shine_end)
...
def predict_genes(sequence: str, start_regex: Pattern, stop_regex: Pattern, shine_regex: Pattern, 
    ...
    while sequence_length - position_courante >= min_gap:
        # Find start codon from current position
        start_pos = find_start(start_regex, sequence, position_courante, sequence_length)
        
        if start_pos is not None:
            # Find stop codon from start position
            stop_pos = find_stop(stop_regex, sequence, start_pos)
            
            if stop_pos is not None:
                # Check minimum gene length (including stop codon)
                gene_length = stop_pos - start_pos + 3
                
                if gene_length >= min_gene_len:
                    # Check for Shine-Dalgarno motif
                    if has_shine_dalgarno(shine_regex, sequence, start_pos, max_shine_dalgarno_distance):
                        # Gene found! Add to list (positions are 1-indexed)
                        probable_genes.append([start_pos + 1, stop_pos + 3])
                        # Move position after the gene + min_gap
                        position_courante = stop_pos + 3 + min_gap
                    else:
                        position_courante = start_pos + 1
                else:
                    position_courante = start_pos + 1
            else:
                position_courante = start_pos + 1
...
def main() -> None: # pragma: no cover
    ...
    # Predict genes in 5' to 3' direction
    ...
    # Get reverse complement and predict genes in 3' to 5' direction
    sequence_rc = reverse_complement(sequence)
    probable_genes_comp_raw = predict_genes(sequence_rc, start_regex, stop_regex, shine_regex, 
                                            args.min_gene_len, args.max_shine_dalgarno_distance, args.min_gap)
    
    # Convert positions from reverse complement to forward strand
    sequence_length = len(sequence)
    probable_genes_comp = []
    for gene in probable_genes_comp_raw:
        # Convert positions: reverse complement position to forward strand position
        start_rc = sequence_length - gene[1] + 1
        stop_rc = sequence_length - gene[0] + 1
        probable_genes_comp.append([start_rc, stop_rc])
    
    # Combine both directions for output
    all_genes = probable_genes + probable_genes_comp
